...
* **v2.4.2** – DisTorchPurgeVRAMV2: new **`Ollama`** toggle (below **`HSWQ`**) unloads every model currently held by the Ollama server used by `comfyui-ollama` (`GET /api/ps` → `POST /api/generate` with `keep_alive=0`, plus `ollama.Client` fallback). Also clears `CHAT_SESSIONS` and deprecated `saved_context` class state in the Comfy node pack. Fallback `nodes/purge_vram.py` kept in sync with root.

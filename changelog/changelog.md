...
* **v2.4.0** – Added automated GPU-wide VRAM management patch executed at startup. Auto-detects non-PyTorch VRAM usage (browsers, Discord, OBS, etc.) via NVML and dynamically adjusts ComfyUI's memory management (General Manage VRAM) at load time. This prevents OOM errors caused by external GPU processes that PyTorch cannot detect, ensuring system-wide memory safety. Added `nvidia-ml-py` dependency.

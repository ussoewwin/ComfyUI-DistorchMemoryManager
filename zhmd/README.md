...
**ComfyUI-VRAM-Manager**（原 ComfyUI-DistorchMemoryManager）是 ComfyUI 的独立显存管理自定义节点。提供 Distorch 显存管理功能，高效处理 GPU/CPU 内存。支持清理 SeedVR2、Qwen3-VL 以及 Nunchaku 模型（FLUX/Z-Image/Qwen-Image）。包含面向 ModelPatchLoader 工作流的 Model Patch Memory Cleaner。通过 NVML 自动检测非 PyTorch 的 VRAM 占用，在多进程环境下防止 OOM。
...
#### Purge VRAM V2 兼容（v1.10，v1.2.0 / v2.0.0 / v2.2.0 增强）

<p align="center">
  <img src="../png/pvram2.png" width="400">
</p>

* **说明**：在 Distorch 套件内恢复 LayerStyle 的 **LayerUtility: Purge VRAM V2**（原版节点），并增强模型卸载、SeedVR2 支持及 Qwen3-VL/Nunchaku 模型清理
* **功能**：UI/行为与原版一致；无需 LayerStyle 即可保留旧工作流。v1.2.0 增强更激进的模型卸载与错误处理。v2.0.0 增加 Qwen3-VL 与 Nunchaku 清理。v2.2.0 增加 Nunchaku SDXL。支持 SeedVR2 DiT/VAE、Qwen3-VL 及 Nunchaku（FLUX/Z-Image/Qwen-Image/SDXL）。
* **输入**：任意类型 (ANY) 透传
* **选项**：
...
* **v2.2.0 增强**：
  ...
  * 在清理顶层参数的同时保留模型结构
* **原因**：上游 LayerStyle 节点消失，在此复刻以保留旧工作流。v1.2.0 改进内存管理。SeedVR2 支持独立缓存系统。v2.0.0 支持 ComfyUI 标准 model_management 未管理的 Qwen3-VL/Nunchaku。v2.2.0 支持需特殊处理的 Nunchaku SDXL。

...
HSWQ INT8 工作流（尤其配合 **HSWQ Batched Detailer**）会留下普通 ComfyUI “unload” 与 DistTorch 通用 purge **无法回收**的 GPU / 主机内存：
...
该开关**不能**替代对所有泄漏类型的完整 ComfyUI 重启；它针对生产中实测到的 HSWQ INT8 + Detailer 残留类别。
...
1. 在 HSWQ INT8 + Batched Detailer 之后清理时（或普通 purge 后 Task Manager 仍显示专用 VRAM 时）启用 Purge VRAM V2 的 **`HSWQ`**。
...
□ PinCache 强制导入路径仍存在
□ 中文版与英文 Release 信息密度一致；代码块保持英文原文
```
